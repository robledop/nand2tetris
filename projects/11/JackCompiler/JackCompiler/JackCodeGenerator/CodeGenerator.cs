...
public class CodeGenerator
{
            ...
            foreach (XmlNode childNode in classVarDec.ChildNodes)
            {
                ...
                if (kind is null)
                {
                    kind = innerText switch
                    {
                        ...
                        _ => throw new JackCompilerException(
                            $"Keyword '{innerText}' not supported as a class variable kind")
                    };
                ...
                {
                    type ??= innerText;
                    ArgumentNullException.ThrowIfNull(childNode.NextSibling);
    ...
    internal string CompileExpression(XmlNode expressionNode)
    {
        ...
        else if (terms?.Count >= 2)
        {
            ...
            var op = 0;
            ArgumentNullException.ThrowIfNull(opTerms);
            for (int i = 0; i < opTerms.Count; i++)
            {
                ...
                else if (identifierNode?.NextSibling?.InnerText.Trim() == "[") // array
                {
                    ...
                    expNode.AppendChild(opTerms[i]?.CloneNode(true) ?? throw new InvalidOperationException());
                ...
                else
                {
                    ...
                    expNode1.AppendChild(opTerms[i]?.CloneNode(true) ?? throw new InvalidOperationException());
        ...
        else if (terms!.Count == 1)
        {
            ...
            if (innerExpressions?.Count > 0 &&
                term.FirstChild?.InnerText.Trim() == "(") // expressions between parenthesis
            {
    ...
    internal string CompileSubroutineCall(XmlNode firstTerm, XmlNodeList expressions)
    {
        ...
        ArgumentNullException.ThrowIfNull(firstIdentifierNode);
        ...
        if (symbol is not null)
        {
            ...
            sb.AppendLine(
                $"call {symbol.Type}.{secondIdentifier?.InnerText.Trim()} {(expressions?.Count ?? 0) + 1}");
        }
        else if (firstIdentifierNode.NextSibling?.InnerText.Trim() == ".")
        {
            var secondIdentifier = identifiers[1];
            sb.AppendLine(
                $"call {firstIdentifierNode.InnerText.Trim()}.{secondIdentifier?.InnerText.Trim()} {expressions?.Count ?? 0}");
        }
        else if (firstIdentifierNode.NextSibling?.InnerText.Trim() == "(")
        {
            sb.AppendLine(
                $"call {_currentClassIdentifier}.{firstIdentifierNode.InnerText.Trim()} {(expressions?.Count ?? 0) + 1}");
        }
