...
public class Parser
{
        ...
        switch (CurrentToken.Type)
        {
            ...
            default:
                throw new JackLexerException(
                    $"A class needs to have variables and/or subroutines at {GetErrorDetails()}");
        }
    }
    ...
    void ParseLetStatement(XmlNode parentNode)
    {
        ...
        {
            throw new JackAnalyzerException(
                $"Expected an equal sign or a left square bracket sign at {GetErrorDetails()}");
        }
    ...
    {
        return
            $"line {CurrentToken.Marker.Line}, position {CurrentToken.Marker.Column}.{Environment.NewLine}{Tokens[Position - 4].Value}{Tokens[Position - 3].Value}{Tokens[Position - 2].Value}{Tokens[Position - 1].Value}{CurrentToken.Value}";
    }
}